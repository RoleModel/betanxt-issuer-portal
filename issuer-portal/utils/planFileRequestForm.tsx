...
// Register the Roboto font only once across invocations.
...
const generatePlanFileRequestPDF = async (
  ...
  const PlanFileRequestPDFDocument: React.FC<
    PlanFileRequestPDFDocumentProps
  > = ({ clientData: documentClientData, logoBase64 }) => {
    ...
    const meetingDateStr = documentClientData?.meetingDate
      ? new Date(documentClientData.meetingDate).toLocaleDateString("en-US", {
          year: "numeric",
          month: "long",
          day: "numeric",
        })
      ...
      <Document>
        <Page size="LETTER" style={styles.page}>
            ...
            {logoBase64 ? (
              ...
              <View style={styles.logoPlaceholder}>
                <Text style={styles.logoText}>
                  {documentClientData?.ticker ?? "LOGO"}
                </Text>
              </View>
            )}
          ...
          <View style={styles.recipient}>
            ...
            <Text style={styles.recipientText}>
              Re: {documentClientData?.issuerName ?? "[COMPANY NAME]"}
            </Text>
          </View>
          ...
          <View style={styles.table}>
            <View style={styles.tableRow}>
              ...
              <Text style={styles.tableValue}>
                {documentClientData?.issuerName ?? "[COMPANY NAME]"}
              </Text>
            </View>
            <View style={styles.tableRow}>
              <Text style={styles.tableLabel}>CUSIP:</Text>
              <Text style={styles.tableValue}>
                {documentClientData?.cusipNumber ?? "[CUSIP]"}
              </Text>
            </View>
            <View style={styles.tableRow}>
          ...
          <Text style={styles.noticeText}>
            ...
            BetaNXT so that the file as of the Record Date would MATCH the
            number you indicate that is to appear in our Proxy Statement.
          </Text>

          <Text style={styles.noticeText}>
            You will be contacted if adjustments must be made by BetaNXT in
            order to match or balance the file you deliver with the figure you
            provide to us as appearing in the Proxy Statement in order to
            maintain the integrity and accuracy of the shareholder record file.
          </Text>
          ...
          <Text style={styles.deliveryText}>
            ...
            portal ONLY, or, password protected file through a secure,
            encrypted, and/or otherwise secure format as pre-established. Please
            note that the Relationship Manager assigned to handle my account is
            Robert Anderson robert.anderson@betanxt.com of Mediant
            Communications, and can also be reached at
            Mediant.Gold.Team@Betanxt.com
          </Text>
          ...
          <View style={styles.signature}>
            ...
            <Text style={styles.signatureName}>
              {documentClientData?.contactName ?? "[AUTHORITY NAME]"}
            </Text>
            <Text style={styles.signatureName}>
              {documentClientData?.issuerName ?? "[COMPANY NAME]"}
            </Text>

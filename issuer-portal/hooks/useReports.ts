...
function _transformNonDtcVoteStatus(nonDtcVoteStatus: unknown): VoteStatusData[] {
  ...
  return [
    ...
    },
    {
      ...
      percentage:
        totalShares > 0 ? (toFiniteNumber(data.printShares) / totalShares) * 100 : 0,
    },
    {
      category: 'IVR',
      ...
      percentage:
        totalShares > 0 ? (toFiniteNumber(data.ivrShares) / totalShares) * 100 : 0,
    },
    {
      category: 'WEB',
      ...
      percentage:
        totalShares > 0 ? (toFiniteNumber(data.webShares) / totalShares) * 100 : 0,
    },
    {
...
function _transformDtcVoteStatus(dtcVoteStatus: unknown): VoteStatusData[] {
  ...
  return [
    ...
    },
    {
      ...
      percentage:
        totalShares > 0 ? (toFiniteNumber(data.votedShares) / totalShares) * 100 : 0,
    },
    {
      category: 'Grand Total',
      shareholders: toFiniteNumber(data.grandTotalShareholders),
...
export function useReports(meetingId?: string) {
  const { data, error, isLoading } = useSWR<ReportsData, Error, ReportsKey | null>(

...
const tryHandleMeetingOverviewPrompt = async ({
  ...
  const meetingType =
    asString(meeting.meetingType) ?? asString(meeting.meeting_type) ?? 'Meeting'
    ...
    case 'meeting': {
      ...
      return {
        ...
        meeting: {
          ...
          meetingType:
            asString(meeting.meetingType) ?? asString(meeting.meeting_type) ?? '',
          meetingDate:
            asString(meeting.meetingDate) ?? asString(meeting.meeting_date) ?? null,
          recordDate:
            asString(meeting.recordDate) ?? asString(meeting.record_date) ?? null,
          mailingDate:
            asString(meeting.mailingDate) ?? asString(meeting.mailing_date) ?? null,

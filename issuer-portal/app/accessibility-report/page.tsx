...
const getAxeToWcagMapping = (
  ...
  rulesData: AxeRulesData;
} => {
  const mapping: Record<string, string> = {};
  const rulesData: AxeRulesData = {};
...
const WCAG_SPEC_URLS: Record<string, string> = {
...
// Load accessibility report + axe rule data
const fetchAccessibilityData = async (): Promise<AccessibilityData> => {
  ...
  const [reportData, rulesData]: [TestResults, AxeRule[]] = await Promise.all([
    reportResponse.json(),
    rulesResponse.json(),
  ]);

  // Handle both 'tests' and 'pages' keys
  const normalizedData: TestResults = {
    ...reportData,
    pages: reportData.pages || reportData.tests || [],
  };

  return { results: normalizedData, axeRules: rulesData };
};

// Helper function to check if a WCAG criterion has been tested
const isCriterionTested = (
  wcagId: string,
...
const AccessibilityReportPage = () => {
  const { data, error } = useSWR<AccessibilityData>(
  ...
  const results: TestResults | null = data
    ? data.results
    : error
      ? { timestamp: fallbackTimestamp, pages: [] }
      : null;
  const axeRules = useMemo<AxeRule[]>(() => data?.axeRules ?? [], [data]);
  ...
  >({});
  const [axeRulesData, setAxeRulesData] = useState<AxeRulesData>({});

  // Format the timestamp after mount to avoid locale/timezone hydration issues
  useEffect(() => {
        ...
        <Stack spacing={2}>
          {pages.length === 0 ? (
            ...
            Object.keys(axeToWcagMapping).length > 0 ? (
              generateCriteriaSummary(results, axeToWcagMapping).map(
                (criteria) => (
                  <CriteriaCard
                    key={criteria.id}
                    criteria={criteria}
                    expanded={!!expandedRules[criteria.id]}
                    onToggleRules={() => {
                      toggleRulesExpanded(criteria.id);
                    }}
                    axeToWcagMapping={axeToWcagMapping}
                    axeRulesData={axeRulesData}
                  />
                )
              )
            ...
            pages.map((test) => <PageResultCard key={test.path} test={test} />)
          )}
        </Stack>
      </Box>
    </Container>
...
const SummaryStats = ({ pages }: SummaryStatsProps) => (
  <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ mb: 4 }}>
    ...
    <Card variant="outlined" sx={{ p: 2, flex: 1 }}>
      ...
      >
        {pages.length -
          pages.filter((test: TestResult) => test.violations.length > 0).length}
      </Typography>
      <Typography variant="body3" color="text.secondary">
        Pages Passed
      </Typography>
    ...
    <Card variant="outlined" sx={{ p: 2, flex: 1 }}>
      ...
      >
        {pages.reduce(
          (total: number, test: TestResult) => total + test.violations.length,
          0
        )}
      </Typography>
      <Typography variant="body3" color="text.secondary">
...
const CriteriaCard = ({
  ...
  const tested = isCriterionTested(criteria.id, axeToWcagMapping);
  const description = getCriteriaDescription(
    criteria.id,
    criteria.violations || [],
    axeToWcagMapping,
    axeRulesData
        ...
        <Box>
          ...
          <Stack spacing={2}>
            {criteria.violations.map((violation, vIndex) => (
              <CriteriaViolationItem key={vIndex} violation={violation} />
            ))}
          </Stack>
        </Box>
      ) : tested ? (
...
interface CriteriaRulesListProps {
  readonly criteria: CriteriaSummary;
  readonly axeToWcagMapping: Record<string, string>;
  readonly axeRulesData: AxeRulesData;
}

const CriteriaRulesList = ({
  criteria,
  axeToWcagMapping,
  axeRulesData,
}: CriteriaRulesListProps) => (
  <Stack spacing={1}>
    {Object.keys(axeToWcagMapping).flatMap((ruleId) => {
      if (axeToWcagMapping[ruleId] !== criteria.id) {
        return [];
      }
      ...
      return (
        ...
        >
          ...
          {passedRule &&
          (passedRule.target !== undefined || passedRule.html !== undefined) ? (
            <Box sx={{ mt: 1 }}>
...
interface CriteriaViolationItemProps {
  readonly violation: CriteriaSummary["violations"][number];
}

const CriteriaViolationItem = ({ violation }: CriteriaViolationItemProps) => (
  <Box
    sx={{
      p: 2,
      border: "1px solid",
...
const PageResultCard = ({ test }: PageResultCardProps) => (
  <Card variant="outlined" sx={{ p: 2 }}>
    <Typography variant="body3" fontWeight="500" gutterBottom>
              ...
              <Box sx={{ mt: 2 }}>
                ...
                <Stack spacing={2}>
                  {violation.elements.map((element, eIndex) => (
                    ...
                    >
                      ...
                      <Typography
                        ...
                        sx={{
                          mb: 1,
                          color: getImpactColorScheme(violation.impact)
                            .textColor,
                        }}
                      >
                        <strong>
                  ...
                  {violation.nodes > (violation.elements?.length ?? 0) && (
                    ...
                    >
                      ... and{" "}
                      {violation.nodes - (violation.elements?.length ?? 0)} more
                      similar elements
                    </Typography>
                  )}
                </Stack>

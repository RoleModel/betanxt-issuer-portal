...
import TaskContextMenu, { type ContextMenuPosition } from "@/components/ui/TaskContextMenu";
...
const TaskDrawer: React.FC<TaskDrawerProps> = ({ open, onClose, task, onTaskUpdate }) => {
  ...
  const { currentMeeting, tasks: _contextTasks, refreshMeetingData: refreshContext } = useMeeting();
  ...
  const handleLinkClick = async (link: TaskLinkWithSignature) => {
    ...
    const isPlanFileRequestTask = currentTask?.title?.toLowerCase().includes("plan file request");
    const isTransferAgentTask = currentTask?.title?.toLowerCase().includes("transfer agent");
    ...
    try {
      ...
      if (taskToSubmit.meetingId) {
        ...
        existingSignedDocument = meetingDocuments.find(
          (doc) =>
            doc.taskId === (taskToSubmit.taskId || taskToSubmit.id) && doc.type === "signed-form",
        );
      }
    ...
    >
      {currentTask || task ? (
        <Stack sx={{ height: "100vh", width: { xs: "100vw", md: 550 } }}>
          {/* Header */}
          <DrawerHeader title={(currentTask ?? task)?.title ?? "Task Details"} onClose={onClose} />
          ...
          <Box sx={{ p: 3 }}>
            <Stack spacing={2}>
              ...
              >
                <Typography variant="body3" fontWeight={500} sx={{ lineHeight: 1.2, mb: 0.5 }}>
                  {(currentTask ?? task)?.title ?? "Task"}
                </Typography>

                <Typography color="text.secondary" variant="body3" sx={{ display: "block", mb: 1 }}>
                ...
                >
                  <StatusChip status={(currentTask ?? task)?.status ?? "INCOMPLETE"} size="small" />
                </Box>
                ...
                void (async () => {
                  ...
                  if (taskToUpdate?.id) {
                    try {
                      ...
                      console.error("Failed to update task after document submission", error);
                    }
                  }
                  ...
                  if (taskToCheck?.phaseNumber === 1) {
                    ...
                    const phase1Tasks = tasks.filter(
                      (t) => t.phaseNumber === 1 && !["BetaNXT", "DFIN"].includes(t.owner ?? ""),
                    );

...
const TaskDrawer: React.FC<TaskDrawerProps> = ({
  ...
  const { updateTaskById, tasks, refetch } = useTasks(
    task?.meetingId || undefined
  );
  const {
    createNewDocument,
    addDocumentHistory,
  ...
  const { isSubmittingTask, setIsSubmittingTask, submitRegularFiles } =
    useTaskSubmission({
      uploadDocument,
      updateTaskById,
      setUploadFiles,
  ...
  const currentClient = clients.find(
    (client) => client.ticker === params.clientTicker
  );
  ...
  useEffect(() => {
    ...
    if (isDTCCAuthorizationTask(taskToCheck)) {
      const isAuthorized =
        taskToCheck?.status === "AUTHORIZED" ||
        taskToCheck?.status === "COMPLETE";
  ...
  useEffect(() => {
    ...
    if (open && taskToUse) {
      try {
        ...
        const hasSignatureLink = fetchedLinks.some(
          (link) => link.action === "signature"
        );
        if (!hasSignatureLink) {
        ...
        const checkSignedDocument = async () => {
          if (taskToUse.meetingId) {
            ...
            const signedDoc = findSignedDocumentForTask(
              taskToUse,
              meetingDocuments
            );
  ...
  const convertDbTaskToTask = (dbTask: DbTask): Task | null => {
    ...
    return {
      ...
      links: taskLinks.map((link: TaskLink) => ({
        label: link.label ?? "",
        url: link.url ?? "",
        action: link.action as
          "download" | "upload" | "sign" | "authorize" | "external",
      ...
      <DocumentViewer
        {...(documentViewerOpen && documentUrl
          ? {
              ...
              fileUrl: documentUrl,
              title:
                approvalTitle ?? (currentTask ?? task)?.title ?? "Document",
              ...
              onSubmitSuccess: () => {
                void (async () => {
                  // Determine appropriate status based on task type
                  let newStatus: components["schemas"]["TaskStatus"] =
                    "COMPLETE";
                  const taskTitle = (
                  ...
                  if (taskToUpdate?.id) {
                    try {
                      await updateTaskById(taskToUpdate.id, {
                        status: newStatus,
                      });
                    } catch (error) {
                      console.error(
                  ...
                  if (taskToCheck?.phaseNumber === 1) {
                    ...
                    if (allPhase1TasksComplete) {
                      ...
                      const meetingTitle =
                        currentMeeting?.title ?? "Shareholder Meeting";
          ...
          : (currentTask || task) && documentViewerOpen
            ? {
                ...
                task: {
                  id: (currentTask ?? task)?.id ?? "",
                  task_id:
                    (currentTask ?? task)?.taskId ?? (currentTask ?? task)?.id,
                  title: (currentTask ?? task)?.title ?? "Document",
                  type: (currentTask || task)?.type,
                  meeting_id: (currentTask || task)?.meetingId,
                },

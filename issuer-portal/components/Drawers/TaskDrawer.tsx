  ...
  const handleDocumentUpload = async (files: File[]) => {
    ...
    if (taskToUse.meetingId) {
      for (const result of uploadResults) {
        // Determine document status based on task status
        let _documentStatus: components['schemas']['DocumentStatus'] = 'UPLOADED'
        if (taskToUse.status === 'SUBMITTED_AWAITING_RECORD_DATE') {
          _documentStatus = 'SUBMITTED_AWAITING_RECORD_DATE'
        }

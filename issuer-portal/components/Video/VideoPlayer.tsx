  ...
  return (
    ...
    >
      ...
      {!isPlaying && showPlayButton ? (
        <PlayButtonOverlay src={src} onTogglePlay={togglePlay} />
      ) : null}
      ...
      <VideoTitle
        ...
        dimmed={!!(src && isPlaying)}
      />

      {showSeries ? <SeriesBadge seriesNumber={seriesNumber} /> : null}
    </Box>
  );
};
...
const PlayButtonOverlay = ({ src, onTogglePlay }: PlayButtonOverlayProps) => (
  <Box
  ...
  >
    <IconButton
      onClick={src ? onTogglePlay : undefined}
      disabled={!src}
      sx={{
        width: 80,
        height: 80,
...
interface VideoControlsProps {
  ...
  readonly onSeek: (event: Event, value: number | number[]) => void;
  readonly onTogglePlay: () => void;
  readonly onToggleMute: () => void;
  readonly onVolumeChange: (event: Event, value: number | number[]) => void;
}
...
const VideoTitle = ({ title, description, dimmed }: VideoTitleProps) => (
  <Box
    sx={{
      position: "absolute",
      top: 24,
...
const SeriesBadge = ({ seriesNumber }: SeriesBadgeProps) => (
  <Box
    sx={(theme) => ({
      position: "absolute",
      top: 6.58,

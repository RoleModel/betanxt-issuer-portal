...
export function TabulationDistributionDrawer({
  ...
  const [notifResult, setNotifResult] = useState<{ ok: boolean; message: string } | null>(
    null
  )
      ...
      >
        ...
        <Box sx={{ flex: 1, overflowY: 'auto', p: 3 }}>
          <Typography variant="body2" color="text.secondary" mb={3}>
            Sends the daily tabulation report to configured recipients during the window
            before the meeting date. Add at least one recipient email below, then enable
            auto-distribution. Reports are delivered each day at 8&nbsp;AM and can be
          ...
          <Stack spacing={3}>
            ...
            {isActive && recipientCount > 0 && (
              <Alert severity="info" icon={<NotificationsActiveIcon fontSize="small" />}>
                ...
                <strong>{(distribution.recipients ?? []).join(', ')}</strong> daily
                starting {distribution.startOffsetDays ?? 15} days before the meeting.
              </Alert>
            )}

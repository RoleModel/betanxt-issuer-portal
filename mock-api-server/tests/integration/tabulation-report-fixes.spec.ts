...
test.describe("Tabulation Report Data Fix Validation", () => {
  ...
  test("CRITICAL: 2025 Wendy's meeting no longer returns all zeros", async ({
    ...
    expect(
      data.positionsVoted.voted,
      "Should have some voted positions"
    ).toBeGreaterThan(0);
    expect(
    ...
    expect(
      data.shareRangePerformance.length,
      "Should have share ranges"
    ).toBeGreaterThan(0);
  ...
  test("VALIDATION: Compare 2025 vs 2026 - should show clear difference", async ({
    ...
    // Get 2026 data (the active meeting has partial voting activity)
    ...
    // Both meetings should have votes, with distinct snapshots for each year.
    expect(
      data2025.positionsVoted.voted,
      "2025 should have voted positions"
    ).toBeGreaterThan(0);
    expect(
      data2026.positionsVoted.voted,
      "2026 should have voted positions"
    ).toBeGreaterThan(0);
    ...
    expect(
      data2026.positionsVoted.votedShares,
      "2026 should have voted shares"
    ).toBeGreaterThan(0);

    expect(data2026.positionsVoted.voted).not.toBe(
      data2025.positionsVoted.voted
    );
    expect(data2026.positionsVoted.votedShares).not.toBe(
      data2025.positionsVoted.votedShares
    );
  ...
  test("REGRESSION: Ensure CSV data loading is working", async ({
    request,
  }) => {
    ...
    const companies = [
      ...
      },
      {
        ...
        name: "Paycom",
        expectedPositions: 3000,
      },
      {
        id: "wwd-annual-meeting-2025",
        name: "Woodward",
        expectedPositions: 3000,
      },
      {
        id: "elvn-annual-meeting-2025",
        name: "Enliven",
    ...
    for (const company of companies) {
      ...
      expect(
        data.setKeys.length,
        `${company.name} should have set keys`
      ).toBeGreaterThan(0);
    }
  });
  ...
  test("EDGE CASE: Mixed voting status validation", async ({ request }) => {
    ...
    expect(data.positionsVoted.votedShares).toBeLessThan(
      data.positionsVoted.totalShares
    );
    ...
    expect(
      participationRate,
      "Participation rate should be realistic"
    ).toBeGreaterThan(10);
    expect(
      participationRate,
      "Participation rate should be realistic"

...
test.describe("Tabulation Report Data Fix Validation", () => {
  ...
  test("CRITICAL: 2025 Wendy's meeting no longer returns all zeros", async ({ request }) => {
    ...
    console.warn(`Total positions: ${data.positionsVoted.voted + data.positionsVoted.unvoted}`);
    ...
    expect(data.positionsVoted.totalShares, "Total shares should not be zero").toBeGreaterThan(0);
    expect(
    ...
    expect(data.positionsVoted.voted, "Should have some voted positions").toBeGreaterThan(0);
    expect(data.positionsVoted.votedShares, "Should have some voted shares").toBeGreaterThan(0);

    // Share ranges should have real data
    expect(data.shareRangePerformance.length, "Should have share ranges").toBeGreaterThan(0);
    const totalRangeShares = data.shareRangePerformance.reduce(
      (sum: number, range: ShareRange) => sum + range.totalShares,
      0,
    );
    expect(totalRangeShares, "Share ranges should have substantial totals").toBeGreaterThan(0);
    ...
    if (data.nonDtcVoteStatus.votedSubtotalShares > 0) {
      expect(totalVotedByChannel, "Should have votes distributed across channels").toBeGreaterThan(
        0,
      );
    }
  });

  test("VALIDATION: Compare 2025 vs 2026 - should show clear difference", async ({ request }) => {
    ...
    expect(data2025.positionsVoted.voted, "2025 should have voted positions").toBeGreaterThan(0);
    expect(data2026.positionsVoted.voted, "2026 should have no voted positions").toBe(0);

    expect(data2025.positionsVoted.votedShares, "2025 should have voted shares").toBeGreaterThan(0);
    expect(data2026.positionsVoted.votedShares, "2026 should have no voted shares").toBe(0);

    // Both should have total shares (positions exist)
    expect(data2025.positionsVoted.totalShares, "2025 should have total shares").toBeGreaterThan(0);
    expect(data2026.positionsVoted.totalShares, "2026 should have total shares").toBeGreaterThan(0);
  });
  ...
  test("REGRESSION: Ensure CSV data loading is working", async ({ request }) => {
    ...
    for (const company of companies) {
      ...
      expect(data.setKeys.length, `${company.name} should have set keys`).toBeGreaterThan(0);
    }
  });
  ...
  test("EDGE CASE: Mixed voting status validation", async ({ request }) => {
    ...
    expect(data.positionsVoted.voted, "Should have some voted positions").toBeGreaterThan(0);
    expect(data.positionsVoted.unvoted, "Should have some unvoted positions").toBeGreaterThan(0);
    ...
    expect(participationRate, "Participation rate should be realistic").toBeGreaterThan(10);

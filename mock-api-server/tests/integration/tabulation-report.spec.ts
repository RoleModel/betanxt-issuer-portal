...
test.describe("Tabulation Report API", () => {
  test.describe("2025 Historical Meetings (should have real voting data)", () => {
    test("Wendy's 2025 Annual Meeting returns real tabulation data", async ({
      request,
    }) => {
    ...
    test("Enliven 2025 Annual Meeting returns real tabulation data", async ({
      request,
    }) => {
      const response = await request.get(
    ...
    test("Paycom 2025 Annual Meeting returns real tabulation data", async ({
      request,
    }) => {
      const response = await request.get(
    ...
    test("Woodward 2025 Annual Meeting returns real tabulation data", async ({
      request,
    }) => {
      const response = await request.get(
  ...
  test.describe("2024 Historical Meetings (synthetic but realistic data)", () => {
    test("2024 Wendy's Annual Meeting has realistic synthetic data", async ({
      request,
    }) => {
      const response = await request.get(
      ...
      const participationRate =
        (data.positionsVoted.votedShares / data.positionsVoted.totalShares) *
        100;
  ...
  test.describe("2026 Active Meetings", () => {
    test("2026 Wendy's Annual Meeting has partial voting data", async ({
      request,
    }) => {
      const response = await request.get(
      ...
      expect(data.positionsVoted.voted).toBeGreaterThan(0);
      expect(data.positionsVoted.unvoted).toBeGreaterThan(0);
  ...
  test.describe("Data Consistency Validation", () => {
    test("Wendy's 2025 data consistency checks", async ({ request }) => {
      ...
      const nonDtcTotal =
        ...
        (data.nonDtcVoteStatus.grandTotalShares -
          data.nonDtcVoteStatus.votedSubtotalShares);
      expect(nonDtcTotal).toBe(data.nonDtcVoteStatus.grandTotalShares);

      const dtcTotal =
        data.dtcVoteStatus.votedShares + data.dtcVoteStatus.unvotedShares;
  ...
  test.describe("Response Schema Validation", () => {
    test("2025 response has all required fields with correct types", async ({
      request,
    }) => {
      const response = await request.get(
        `${API_BASE_URL}/api/meetings/wen-annual-meeting-2025/tabulation-report`
      ...
      expect(new Date(data.lastCalculatedAt).toISOString()).toBe(
  ...
  test.describe("Performance Tests", () => {
    test("API responds within reasonable time for large dataset", async ({
      request,
    }) => {

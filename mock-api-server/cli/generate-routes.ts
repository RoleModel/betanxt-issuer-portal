...
function generateRouteContent(route: RouteInfo, domainModels: DomainModelMap): string {
  ...
  const someMethodUsesPathParams = route.methods.some((method) => methodUsesPathParams(method));
...
import { handleCors, withCors } from '@/utils/cors'`;
  ...
  for (const method of route.methods) {
    ...
    if (domainMapping) {
      const result = generateDomainModelCall(method, domainMapping.functionName, params, route);

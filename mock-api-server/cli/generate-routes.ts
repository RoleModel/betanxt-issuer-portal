  ...
  if (method === 'GET' && functionName.includes('list')) {
    ...
    if (functionName === 'listPositions') {
      ...
      if (filterParams.length > 0) {
        const filters = filterParams.map((p) => {
...
function generateRouteContent(route: RouteInfo, domainModels: DomainModelMap): string {
  ...
  const someMethodUsesPathParams = route.methods.some(method => {
    ...
    return hasPathParams && methodMapping && (
      ...
      methodMapping.functionName === 'listAccountUsers' ||
      methodMapping.functionName === 'listUserAccounts' ||
      methodMapping.functionName === 'createDocument' ||
      methodMapping.functionName === 'createProposal' ||
      methodMapping.functionName === 'createTask' ||
  ...
  const needsNextRequest = route.methods.some(method => {
    ...
    return methodMapping && (method !== 'GET' || hasQueryParams || (hasPathParams && someMethodUsesPathParams))
  })

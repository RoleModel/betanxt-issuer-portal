...
async function getMeetingEmailData(
  ...
  return { proposals, totalSharesEligible, totalSharesVoted: Math.round(totalSharesVoted), quorumRequired, quorumMet }
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    let query = supabase
      .from('meeting')
      .select('id, ticker, meeting_date, tabulation_distribution, title, meeting_type, total_shares_outstanding, quorum_requirement')
    ...
    if (meetingsError) {
      return withCors(
        NextResponse.json({ error: 'Failed to fetch meetings', message: meetingsError.message }, { status: 500 })
      )
    }
    ...
    for (const meeting of meetings ?? []) {
      ...
      if (!dist?.enabled) {
        results.push({ meetingId, ticker, notificationsCreated: 0, emailsSent: 0, emailRecipients: [], skipped: 'distribution disabled' })
        continue
      }
      ...
      if (!force && !isInWindow(meetingDate, offsetDays)) {
        results.push({ meetingId, ticker, notificationsCreated: 0, emailsSent: 0, emailRecipients: [], skipped: 'outside distribution window' })
        continue
      }

      if (!force && alreadySentToday(dist.lastSentAt)) {
        results.push({ meetingId, ticker, notificationsCreated: 0, emailsSent: 0, emailRecipients: [], skipped: 'already sent today' })
        continue
      }
      ...
      results.push({ meetingId, ticker, notificationsCreated, emailsSent, emailRecipients: recipients })
    }
